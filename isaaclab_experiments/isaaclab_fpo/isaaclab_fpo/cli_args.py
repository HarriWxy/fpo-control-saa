...
def add_fpo_args(parser: argparse.ArgumentParser):
    ...
    arg_group.add_argument(
        ...
        help=(
            ...
            "joint-latent Gaussian PPO variant; 'fsppo_joint_endpoint' "
            "selects the terminal-action MAE comparison; omitted keeps the "
            "task default."
        ),
    )
...
def update_fpo_cfg(agent_cfg: FpoRslRlOnPolicyRunnerCfg, args_cli: argparse.Namespace):
    ...
    elif algorithm_variant == "fsppo_joint_endpoint":
        agent_cfg.policy.class_name = "PMFActorCritic"
        agent_cfg.algorithm.class_name = "FSPPOJointEndpoint"
        # This comparison uses only the one-NFE terminal action.  It has no
        # transport-map budget and therefore needs no map-sample setting.
        agent_cfg.policy.sampling_steps = 1
        agent_cfg.algorithm.schedule = "fixed"
        agent_cfg.algorithm.knn_entropy_coef = 0.0
        agent_cfg.algorithm.trust_region_mode = "ppo"
        agent_cfg.algorithm.fsppo_joint_enable_budget = False
        if args_cli.experiment_name is None:
            agent_cfg.experiment_name = (
                f"{agent_cfg.experiment_name}_fsppo_joint_endpoint"
            )
